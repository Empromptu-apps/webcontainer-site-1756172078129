        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">My Tiny Habits</h1>
            </div>
            <div className="flex items-center space-x-4">
              <button
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {habits.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">No habits yet</h3>
          ...
          <div className="space-y-6">
            {habits.map((habit) => {
              ...
              return (
                <div key={habit.id} className="card-luxury p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
                        After I <span className="text-primary-600 dark:text-primary-400">{habit.anchor_moment}</span>, 
                        I will <span className="text-primary-600 dark:text-primary-400">{habit.new_habit}</span>
                        {habit.celebration && (
                          <span> and celebrate by <span className="text-primary-600 dark:text-primary-400">{habit.celebration}</span></span>
                        )}
                      </h3>
                    </div>
                    <button
                      ...
                      {weekDates.map((date, index) => {
                        ...
                        return (
                          <div key={dateStr} className="text-center">
                            <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium">{dayName}</div>
                            <div className="text-xs text-gray-400 dark:text-gray-500 mb-2">{date.getDate()}</div>
                            <button
                  ...
                  <div className="flex flex-wrap gap-6 text-sm">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-gray-600 dark:text-gray-400">
                        <span className="font-semibold text-gray-900 dark:text-gray-100">This Week:</span> {metrics.weeklyPercentage}%
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-green-500 rounded-full"></div>
                      <span className="text-gray-600 dark:text-gray-400">
                        <span className="font-semibold text-gray-900 dark:text-gray-100">Current Streak:</span> {metrics.currentStreak} days
                      </span>
                    </div>
                  </div>
                </div>
